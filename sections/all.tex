...
Nix is analogous to how a pure function works. A pure function has no hidden state. Regardless of where, how, or when the pure function is executed, it will, for the same input, return the same output.
In your Nix expression, you list all of your required inputs, and when evaluated it produces a \lstinline{.drv} (derivation) file .
In Nix, a derivation file is the low-level build instruction that Nix actually executes, a sort of recipe that contains the following:
