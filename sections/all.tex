...
To understand the problem we want to solve, we also need to understand what Nix and NixOS is.
...
The build inputs all appear in the final store path of the artifact under \lstinline{/nix/store/<hash>-<name>}, and the unique hash is calculated from all the build inputs. If you change something small, such as an environment variable, the inputs will be altered and thus the hash of the build artifact will be changed. This ensures reproducible and deterministic builds. If the store path is identical, the contents are identical as well. 
...
\subsubsection{Embedding Git commit into Nix expression}

One might think that you can solve this problem of mapping between store path and git commit at when builing the NixOS system image. Including the current commit hash as a build input would mean that every commit produces a different store path. This would make it impossible to see that a server actually is unchanged over time.
