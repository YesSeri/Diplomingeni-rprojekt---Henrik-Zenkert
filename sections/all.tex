...
Diagram~\ref{fig:diagram-runtime} show that the PHP tool DBC Digital uses requires SSH access to all servers. This is a security concern. If the CI server is compromised, an attacker could gain privileged access to every host via the CI server, effectively bypassing firewall protections.
...
In addition to this recursive issue, we would also not be able to verifiy that a host has not changed between to commits if we embed the commit hash directly into the derivation, which is a key requirement for DBC Digital.
...
\begin{itemize}
	...
	\item Performance, must work well when deployed to 200 servers, that will be activated on a maximum an average of once per day.
