...
cryptography==2.0.0
\end{lstlisting}
\caption{An example \texttt{requirements.txt} file that appears reproducible but does not capture system-level dependencies, interpreter versions, or platform differences.}
\label{lst:reqfile}
\end{figure}
...
The project began with a face where I gathered requirements. The first steps was looking at the current solution and understanding its limitations and how it worked.

In collaboration with the infrastructure team at DBC digital in particular Adam and Sarah, we worked out the requirements for the project to be considered successful. Among these criterias were eliminating the SSH access from the CI server to the server fleet, reducing delays in UI updates, and enabling some extensibility through the use of arbitrary metadata.
...
\subsection{Iterative Implementation Strategy}
