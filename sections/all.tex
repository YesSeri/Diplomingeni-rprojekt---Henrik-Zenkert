...
\centering
\begin{lstlisting}
WITH latest AS (SELECT DISTINCT ON (ac.hostname)
...
The most complex SQL logic, Figure \ref{fig:diffcult-sql}, is the logic for matching the hosts with the latest store path to commit mapping.

First running the query manually and then local integration tests were created with a dummy database to enable testing some of the more complex SQL queries. In particular the one in \ref{fig:diffcult-sql}
