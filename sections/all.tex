...
\centering
\begin{lstlisting}
WITH latest AS (SELECT DISTINCT ON (a.hostname)
  a.activated_at, a.store_path, a.hostname
FROM activation a
  ORDER BY a.hostname, a.activated_at DESC
) 
SELECT DISTINCT ON(l.hostname) 
  l.activated_at, l.store_path, l.hostname,
  ngl.commit_hash, ngl.branch
FROM latest l LEFT JOIN nix_git_link ngl 
  ON ngl.store_path = l.store_path 
  ORDER BY 
    l.hostname, ngl.branch = 'master' desc, ngl.linked_at ASC;
\end{lstlisting}
\caption{Complex SQL logic tested through integration tests}
\label{fig:diffcult-sql}
\end{figure}
...
The most complex SQL logic, Figure \ref{fig:diffcult-sql}, is the logic for matching the hosts with the latest store path to commit mapping, the result of which can be seen in Figure \ref{fig:match-result}. The SQL query identifies, for each host, the most recent activation using the \lstinline{WITH latest}, CTE (Common Table Expression). It then selects the commit mapping with the earliest \lstinline{linked_at} timestamp, that corresponds to selected activation's store path, while always prioritizing mappings with branch named \texttt{master} over other branches.

To ensure correctness when working with such complex logic I needed a reliable way to test if my query is correct. I wrote a database seeding script that seeds the database with dummy data, where I have clearly marked the data the should be chosen.

---
VErsion 2


The most complex logic in the back end is matching the each host with its latest activation with the correct Git commit (revision) and branch, (see Appendix~\ref{appendix:matching-sql}). To do this we need to combine multiple tables, sorting in specific order, selecting the first, sometimes prioritizing if in a collection of commits a certain branch name exists.  Unit tests were insufficient to test correctness, since we need data in a database to actually test the query.

To validate the SQL query I created a seeding script with dummy data. The actual  for the database with hosts, and then validated it in the UI. 
